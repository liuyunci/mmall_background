...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> register(User user){
        ...
        int resultCount = userMapper.insert(user);//插入行数
        if(resultCount == 0){
    ...
    public ServerResponse<String> checkAnswer(String username,String question,String answer){
        ...
        return ServerResponse.createBySuccessMessage("答案不正确");
    }
    ...
    public ServerResponse<String> resetPassword(String passwordOld,String passwordNew,User user){
        //防止横向越权,要校验一下这个用户的旧密码,一定要指定是这个用户,因为我们会查询一个count(1),如果不指定id,那么查出来的count肯定大于0
    ...
    public ServerResponse<User> updateInformation(User user){
        ...
        if(resultCount > 0){
            return ServerResponse.createByErrorMessage("email已经存在,请更换email再尝试跟新");
        }
